    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4 lg:gap-8">
            ...
            <div class="flex items-center gap-4 flex-shrink-0">
                <img src="{{ asset('asset/img/trickleup-favicon.png') }}" class="h-10 w-auto drop-shadow-lg"
                    alt="Trickle Up Logo">
            </div>
                ...
                <div class="relative group">
                    <form action="{{ route('dashboard') }}" method="GET" class="relative">
                        ...
                        <input type="text" name="search" x-model="query" @input.debounce.300ms="search()"
                            @focus="if(query.length >= 2) showResults = true" placeholder="Search in Trickle Up..."
                            autocomplete="off"
                    ...
                    <div x-show="showResults" @click.away="showResults = false" x-transition
                        ...
                        <div x-show="loading" class="p-4 flex justify-center">
                            <svg class="animate-spin h-6 w-6 text-primary-500" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div class="overflow-y-auto" x-show="!loading">
                            <!-- No Results -->
                            <div x-show="results.folders.length === 0 && results.files.length === 0"
                            ...
                            <div class="py-2">
                                ...
                                <template x-for="folder in results.folders" :key="'folder-'+folder.id">
                                    <a :href="folder.url"
                                        class="flex items-center gap-4 px-5 py-3 hover:bg-white/5 transition-colors group">
                                        <div
                                            class="p-2 rounded-lg bg-secondary-500/10 text-secondary-500 group-hover:bg-primary-500/20 group-hover:text-primary-500 transition-colors">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-bold text-white truncate"
                                                x-html="highlight(folder.name)"></p>
                                ...
                                <template x-for="file in results.files" :key="'file-'+file.id">
                                    <a :href="file.url" target="_blank"
                                        class="flex items-center gap-4 px-5 py-3 hover:bg-white/5 transition-colors group">
                                        <div
                                            class="p-2 rounded-lg bg-white/5 text-gray-400 group-hover:bg-primary-500/20 group-hover:text-primary-500 transition-colors">
                                            ...
                                            <template
                                                x-if="!file.mime_type.startsWith('image/') && !file.mime_type.startsWith('video/') && file.mime_type !== 'application/pdf'">
                        ...
                        <div class="border-t border-white/10 p-3 flex items-center justify-between bg-white/5">
                            <a :href="'{{ route('dashboard') }}?search=' + encodeURIComponent(query)"
                                class="text-xs font-bold text-primary-500 hover:underline">Advanced search</a>
                            <a :href="'{{ route('dashboard') }}?search=' + encodeURIComponent(query)"
                                class="flex items-center gap-2 text-xs font-bold text-primary-500 hover:underline">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M8 7l4 4m0 0l4-4m-4 4V3"></path>
                                </svg>
                                All results
                            </a>
                        </div>
    ...
    <div x-data="{ uploading: false, uploadMinimized: false, uploadFiles: [], viewMode: localStorage.getItem('viewMode') || 'list',
        ...
        <div x-init="$watch('viewMode', val => localStorage.setItem('viewMode', val))"
            ...
            <div x-show="uploading" style="display: none;"
                ...
                <!-- Header -->
                <div @click="uploadMinimized = !uploadMinimized"
                    class="bg-[#323232] text-white px-4 py-3 flex items-center justify-between cursor-pointer rounded-t-lg h-[48px] flex-shrink-0">
                    <h3 class="text-[15px] font-medium"
                        x-text="uploadFiles.some(f => f.status === 'uploading') ? ('Uploading ' + uploadFiles.length + (uploadFiles.length === 1 ? ' item' : ' items')) : (uploadFiles.length + (uploadFiles.length === 1 ? ' upload complete' : ' uploads complete'))">
                    </h3>
                    <div class="flex items-center gap-2">
                        <button class="hover:bg-dark-card/10 p-1 rounded transition-colors"
                            @click.stop="uploadMinimized = !uploadMinimized">
                            <svg class="w-5 h-5 transition-transform duration-200"
                                :class="uploadMinimized ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <button @click.stop="uploading = false; uploadFiles = []"
                            class="hover:bg-dark-card/10 p-1 rounded transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                ...
                <div x-show="!uploadMinimized" class="flex flex-col flex-1 overflow-hidden" x-transition.opacity>
                    <!-- Sub-header -->
                    <div
                        class="bg-[#f8f9fa] border-b border-white/10 px-4 py-2.5 flex items-center justify-between flex-shrink-0">
                        <span class="text-sm text-gray-400"
                            x-text="uploadFiles.some(f => f.status === 'uploading') ? 'Starting uploads...' : 'Uploads complete'"></span>
                        <button x-show="uploadFiles.some(f => f.status === 'uploading')"
                            @click="uploading = false; uploadFiles = []"
                            class="text-primary-500 text-[13px] font-semibold hover:text-primary-700 uppercase tracking-wider">Cancel</button>
                    </div>
                    ...
                    <div class="overflow-y-auto bg-dark-card flex-1 min-h-[60px]">
                        <template x-for="file in uploadFiles" :key="file.name">
                            <div
                                class="px-4 py-3 flex items-center justify-between border-b border-white/10 last:border-0 hover:bg-white/5">
                                <div class="flex items-center gap-3 overflow-hidden">
                                    <!-- Dummy image icon -->
                                    <div
                                        class="w-6 h-6 bg-red-50 rounded flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex-shrink-0 ml-3">
                                    <!-- Spinning Circle -->
                                    <svg x-show="file.status === 'uploading'" class="w-5 h-5 text-gray-300 animate-spin"
                                        fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                            stroke-width="4"></circle>
                                        <path class="opacity-75" fill="#1a73e8"
                                            d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                                        </path>
                                    </svg>
                                    <!-- Checkmark -->
                                    <svg x-show="file.status === 'done'" class="w-5 h-5 text-green-500" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M5 13l4 4L19 7"></path>
                                    </svg>
                                    <!-- Error -->
                                    <svg x-show="file.status === 'error'" class="w-5 h-5 text-red-500" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
            ...
            <div id="dashboard-sidebar" class="w-full md:w-64 flex-shrink-0">
                ...
                <div class="relative mb-6" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition style="display: none;"
                        ...
                        <form action="{{ route('files.upload') }}" method="POST" enctype="multipart/form-data"
                            class="m-0" @submit.prevent="submitAction($event)">
                            @csrf
                ...
                <nav class="space-y-1">
                    <a href="{{ route('dashboard') }}"
                        class="flex items-center gap-3 px-4 py-2.5 {{ (isset($currentTab) && $currentTab === 'my_drive') ? 'bg-primary-500/10 text-primary-500' : 'text-gray-400 hover:bg-white/10 hover:text-white' }} rounded-xl font-medium transition-colors">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <a href="{{ route('recent') }}"
                        class="flex items-center gap-3 px-4 py-2.5 {{ (isset($currentTab) && $currentTab === 'recent') ? 'bg-primary-500/10 text-primary-500' : 'text-gray-400 hover:bg-white/10 hover:text-white' }} rounded-xl font-medium transition-colors">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('starred') }}"
                        class="flex items-center gap-3 px-4 py-2.5 {{ (isset($currentTab) && $currentTab === 'starred') ? 'bg-primary-500/10 text-primary-500' : 'text-gray-400 hover:bg-white/10 hover:text-white' }} rounded-xl font-medium transition-colors">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('trash') }}"
                        class="flex items-center gap-3 px-4 py-2.5 {{ (isset($currentTab) && $currentTab === 'trash') ? 'bg-primary-500/10 text-primary-500' : 'text-gray-400 hover:bg-white/10 hover:text-white' }} rounded-xl font-medium transition-colors">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="flex items-center gap-2 text-sm font-medium text-gray-500">
                            ...
                            @else
                                <a href="{{ route('dashboard') }}" class="hover:text-primary-500 transition-colors">My
                                    Drive</a>
                            ...
                            @forelse($folders as $folder)
                                <div
                                    ...
                                    <a href="{{ route('dashboard', ['folder' => $folder->id]) }}"
                                        ...
                                        <div
                                            class="p-2 rounded-lg bg-secondary-500/10 group-hover:bg-primary-100 transition-colors">
                                        ...
                                        <div class="flex-1 min-w-0 pr-8">
                                            ...
                                            <p class="text-xs text-gray-500 truncate mt-0.5">{{ $folder->files_count ?? 0 }}
                                                items
                                            </p>
                                        </div>
                                    </a>
                                    ...
                                    <div class="absolute top-4 right-2"
                                        x-data="{ actionOpen: false, isStarred: {{ $folder->is_starred ? 'true' : 'false' }} }">
                                        ...
                                        <div x-show="actionOpen" x-transition style="display: none;"
                                            class="absolute right-0 z-10 mt-1 w-40 bg-dark-card rounded-xl shadow-lg border border-white/10 py-1">
                                            @if(isset($currentTab) && $currentTab === 'trash')
                                                <form action="{{ route('folders.restore', $folder->id) }}" method="POST"
                                                    @submit.prevent="submitAction($event)">
                                                    @csrf
                                                    <button type="submit"
                                                        class="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-white/5">
                                                ...
                                                <form action="{{ route('folders.force_delete', $folder->id) }}" method="POST"
                                                    data-confirm="Permanently delete this folder?"
                                                    @submit.prevent="submitAction($event)">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        class="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                                            ...
                                            @else
                                                <button
                                                    @click="$dispatch('open-rename-modal', { id: {{ $folder->id }}, name: '{{ addslashes($folder->name) }}', type: 'folder' }); actionOpen = false"
                                                    class="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-white/5 hover:text-primary-500">
                                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                ...
                                                <button type="button"
                                                    @click="toggleStar('{{ route('folders.star', $folder->id) }}', (val) => isStarred = val)"
                                                    class="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-200 hover:bg-white/5">
                                                    <svg class="w-4 h-4" :class="isStarred ? 'text-yellow-400' : 'text-gray-400'"
                                                ...
                                                <form action="{{ route('folders.delete', $folder->id) }}" method="POST"
                                                    data-confirm="Move this folder to trash?"
                                                    @submit.prevent="submitAction($event)">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        class="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                            ...
                            <table class="min-w-full divide-y divide-white/5">
                                <tbody class="bg-dark-card divide-y divide-white/5">
                                    @foreach($folders as $folder)
                                        <tr x-data="{ isStarred: {{ $folder->is_starred ? 'true' : 'false' }} }"
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div
                                                    ...
                                                    @if(isset($currentTab) && $currentTab === 'trash')
                                                        <form action="{{ route('folders.restore', $folder->id) }}" method="POST"
                                                            class="inline-block m-0" @submit.prevent="submitAction($event)">
                                                            @csrf
                                                            <button type="submit"
                                                                class="text-gray-400 hover:text-primary-500 p-2 rounded-lg hover:bg-primary-50 transition-colors"
                                                                title="Restore">
                                                                <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                                    viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        ...
                                                        <form action="{{ route('folders.force_delete', $folder->id) }}"
                                                            method="POST" data-confirm="Permanently delete this folder?"
                                                            class="inline-block m-0" @submit.prevent="submitAction($event)">
                                                            @csrf @method('DELETE')
                                                            <button type="submit"
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            @click="toggleStar('{{ route('folders.star', $folder->id) }}', (val) => isStarred = val)"
                                                            :class="isStarred ? 'text-yellow-400' : 'text-gray-400'"
                                                            class="hover:text-yellow-500 p-2 rounded-lg hover:bg-yellow-50 transition-colors inline-block m-0"
                                                            title="Toggle Star">
                                                            <svg class="w-5 h-5" :fill="isStarred ? 'currentColor' : 'none'"
                                                                stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                                                    d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z">
                                                        ...
                                                        <button
                                                            @click="$dispatch('open-rename-modal', { id: {{ $folder->id }}, name: '{{ addslashes($folder->name) }}', type: 'folder' })"
                                                            class="text-gray-400 hover:text-primary-500 p-2 rounded-lg hover:bg-primary-50 transition-colors"
                                                            title="Rename">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                                viewBox="0 0 24 24">
                                                        ...
                                                        <form action="{{ route('folders.delete', $folder->id) }}" method="POST"
                                                            data-confirm="Move this folder to trash?" class="inline-block m-0"
                                                            @submit.prevent="submitAction($event)">
                                                            @csrf @method('DELETE')
                                                            <button type="submit"
                                                                class="text-gray-400 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 transition-colors"
                        ...
                        <table class="min-w-full divide-y divide-white/5">
                            ...
                            <tbody class="bg-dark-card divide-y divide-white/5">
                                @forelse($files as $file)
                                    <tr x-data="{ isStarred: {{ $file->is_starred ? 'true' : 'false' }} }"
                                        class="hover:bg-white/5 transition-colors group">
                                        <td class="px-4 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div
                                                ...
                                                @if(isset($currentTab) && $currentTab === 'trash')
                                                    ...
                                                    <form action="{{ route('files.force_delete', $file->id) }}" method="POST"
                                                        data-confirm="Permanently delete this file?" class="inline-block m-0"
                                                        @submit.prevent="submitAction($event)">
                                                        @csrf @method('DELETE')
                                                        <button type="submit"
                                                            class="text-gray-400 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 transition-colors"
                                                ...
                                                @else
                                                    ...
                                                    <button type="button"
                                                        @click="toggleStar('{{ route('files.star', $file->id) }}', (val) => isStarred = val)"
                                                        :class="isStarred ? 'text-yellow-400' : 'text-gray-400'"
                                                        class="hover:text-yellow-500 p-2 rounded-lg hover:bg-yellow-50 transition-colors inline-block m-0"
                                                        title="Toggle Star">
                                                        <svg class="w-5 h-5" :fill="isStarred ? 'currentColor' : 'none'"
                                                    ...
                                                    <button
                                                        @click="$dispatch('open-rename-modal', { id: {{ $file->id }}, name: '{{ addslashes($file->name) }}', type: 'file' })"
                                                        class="text-gray-400 hover:text-primary-500 p-2 rounded-lg hover:bg-primary-50 transition-colors"
                                                        title="Rename">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                    ...
                                                    <form action="{{ route('files.delete', $file->id) }}" method="POST"
                                                        data-confirm="Move this file to trash?" class="inline-block m-0"
                                                        @submit.prevent="submitAction($event)">
                                                        @csrf @method('DELETE')
                                                        <button type="submit"
                                                            class="text-gray-400 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 transition-colors"
                            ...
                            <div x-data="{ isStarred: {{ $file->is_starred ? 'true' : 'false' }} }"
                                ...
                                <a href="{{ asset('storage/' . $file->path) }}" target="_blank"
                                    ...
                                    <div class="text-center w-full px-2">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">{{ number_format($file->size / 1024, 2) }} KB
                                        </p>
                                    </div>
                                </a>
                                <div
                                    class="border-t border-white/10 p-2 flex items-center justify-around opacity-0 group-hover:opacity-100 transition-opacity bg-white/5/50 rounded-b-xl">
                                    @if(isset($currentTab) && $currentTab === 'trash')
                                        <form action="{{ route('files.restore', $file->id) }}" method="POST" class="m-0"
                                            @submit.prevent="submitAction($event)">
                                            @csrf
                                            <button type="submit"
                                                class="p-1.5 text-gray-500 hover:text-primary-500 rounded hover:bg-dark-card transition-colors"
                                                title="Restore">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path>
                                        ...
                                        <form action="{{ route('files.force_delete', $file->id) }}" method="POST"
                                            data-confirm="Permanently delete this file?" class="m-0"
                                            @submit.prevent="submitAction($event)">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="p-1.5 text-gray-500 hover:text-red-600 rounded hover:bg-dark-card transition-colors"
                                                title="Delete Forever">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                    ...
                                    @else
                                        ...
                                        <button type="button"
                                            @click="toggleStar('{{ route('files.star', $file->id) }}', (val) => isStarred = val)"
                                            :class="isStarred ? 'text-yellow-400' : 'text-gray-500'"
                                            class="p-1.5 hover:text-yellow-500 rounded hover:bg-dark-card transition-colors m-0"
                                            title="Toggle Star">
                                        ...
                                        <form action="{{ route('files.delete', $file->id) }}" method="POST"
                                            data-confirm="Move this file to trash?" class="m-0"
                                            @submit.prevent="submitAction($event)">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="p-1.5 text-gray-500 hover:text-red-600 rounded hover:bg-dark-card transition-colors"
        ...
        <div x-data="{ show: false }" @open-folder-modal.window="show = true" x-show="show" style="display: none;"
            ...
            <div x-show="show" x-transition.scale.origin.center
                ...
                <form action="{{ route('folders.create') }}" method="POST"
                    @submit.prevent="submitAction($event, () => show = false)">
                    @csrf
                    <input type="hidden" name="parent_id" value="{{ $parentId }}">
        ...
        <!-- Rename Modal -->
        <div x-data="{ show: false, itemId: null, itemName: '', itemType: '', actionUrl: '' }"
            @open-rename-modal.window="
            show = true; 
        ...
        <div x-data="{ toasts: [], addToast(toast) { const id = Date.now() + Math.random(); this.toasts = [...this.toasts, { ...toast, id }]; setTimeout(() => { this.toasts = this.toasts.filter(t => t.id !== id); }, 3000); } }"
            ...
            <template x-for="toast in toasts" :key="toast.id">
                <div x-transition.opacity.duration.300ms
                    ...
                    <svg x-show="toast.type === 'success'" class="w-5 h-5 text-green-500 flex-shrink-0" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                    <svg x-show="toast.type === 'warning'" class="w-5 h-5 text-yellow-500 flex-shrink-0" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
